...
<form id="product_form" method="POST" enctype="multipart/form-data" action={{ $action }}>
	...
	<input type="hidden" id="product_index_route" value="{{ route('product.index') }}" >
				...
				<div class="form-group">
					<select class="form-control show-tick selectPick" name="brand_id" id="brand_id" required>
						...
						<option value="{{ $brand->brand_id }}"
							@if ($brand->brand_id == old('brand_id', $product->brand_id))
							selected="selected"
							@endif
							>
							{{ $brand->brand_name }}
				...
				<div class="form-group">
					<select class="form-control show-tick selectPick" name="category_id" id="category_id" required>
						...
						<option value="{{ $category->category_id }}"
							@if ($category->category_id == old('category_id',$product->category_id))
							selected="selected"
							@endif
							>
		...
		<div class="row clearfix">
			<div class="col-sm-6">
				<div class="form-group" id="sub_category">
					<select class="form-control show-tick selectPick selectSubCategory" name="sub_category_id" id="sub_category_id" required>
									...
									@if(isset($category->subCategory))
								  	 @foreach($category->subCategory as $sub_category)
								  	  <option value="{{ $sub_category->sub_category_id }}"
												@if ($sub_category->sub_category_id == old('sub_category_id',$product->sub_category_id))
											    selected="selected"
											  @endif
												>
			...
			<div class="col-sm-6">
				<div class="form-group" id="sub_sub_category">
					<select class="form-control show-tick selectPick selectSubSubCategory" name="sub_sub_category_id" id="sub_sub_category_id" required>
									...
									@foreach($category->subCategory as $sub_category)
										@if ($sub_category->sub_category_id == old('sub_category_id',$product->sub_category_id))
										  @if(isset($sub_category->subSubCategory))
										   @foreach($sub_category->subSubCategory as $sub_sub_category)
											  <option value="{{ $sub_sub_category->sub_sub_category_id }}"
													@if ($sub_sub_category->sub_sub_category_id == old('sub_sub_category_id',$product->sub_sub_category_id))
												    selected="selected"
												  @endif
													>
					...
					@if ($errors->formError->has('sub_sub_category_id'))
	  ...
	  <div class="row clearfix">
	  	<div class="col-sm-6">
	  		<div class="form-group form-float">
	  			...
	  			<div class="help-info">Product Image<span class="col-pink">{{ !empty($is_edit) ? '' : '*' }}</span>
	  			</div>
	  	...
	  	<div class="col-sm-2">
	  		<div class="form-group">
	  			...
	  			{{ (empty($product->in_stock) || $product->in_stock == 1) ? 'checked' : '' }} >
		...
		<div class="row clearfix">
			<div class="col-sm-6">
				<div class="form-group form-float">
					<div class="form-line {{ $errors->formError->has('product_description') ? ' focused error' : '' }}">
						<br/>
						<textarea name="product_description" class="form-control tinymce" 
	...
	<fieldset>
		@if(isset($is_edit))
		@include('product.quantity_edit_form')
		@else
	...
	<fieldset>
		<!-- Product Additional Image Starts -->
		<div class="row clearfix">
			<div class="col-sm-12">
				<div class="form-group form-float">
					@if(isset($is_edit))
		...
		<div class="row clearfix">
			<div class="col-sm-6">
				<div class="form-group form-float">
					<div class="form-line {{ $errors->formError->has('product_description') ? ' focused error' : '' }}">
						<br/>
						<textarea name="product_other_description" class="form-control tinymce" 
			...
			<div class="col-md-6">
				...
				<div class="form-group form-float">
					<div class="form-line">
						<input type="text" class="form-control" name="meta_desc" value="{{ old('meta_desc' , $product->meta_desc) }}">
		...
		<div class="row clearfix">
			<div class="col-md-6">
				<div class="form-group form-float">
					@if(isset($is_edit))
					<button class="btn bg-green waves-effect" type="submit" name="update_btn" id="update_btn">{{ $submit_text }}</button>
					@else
						<button class="btn bg-green waves-effect" type="submit" name="submit_btn" id="submit_btn">{{ $submit_text }}</button>
					@endif
				</div>
			</div>
		</div>
		...
		<!-- Product Name / Image / Additional Image Starts -->
	</fieldset>
